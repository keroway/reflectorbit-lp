...
策定当初（Track A/B 着手前）は LP に動的な映像はなかった（Hero はインライン SVG + SMIL
アニメーション、How to Play は静的 SVG 図解）。以下 2 系統で動画を導入する計画を立て、
現在は両トラックとも実装済み: Track B（How to Play 動画）は完了済み、Track A（Trailer）は
セクション実装済みでプレースホルダ動画（スクリーンショット3枚のスライドショー）が
コミット済み、実写素材への差し替えのみ #85 待ち。
...
扱う。`scripts/gen-og.mjs` / `pnpm run og:gen` と同じ運用（ローカルで生成 → コミット、
Cloudflare Pages の `pnpm run build` では再生成しない）に揃える。
...
- 現状 `src/pages/index.astro` にインライン SVG で実装されている「Space で軌道半径を縮小 /
  ...
  （または併設する）。
- 「Space で半径を縮める / Shift で広げる」操作対比は静止図より動画の方が伝わりやすく、
  このセクションが最も動画化の価値が高いと判断（Hero 背景ループの動画化は
  価値対コストが低いため対象外、末尾の「将来検討」参照）。
  既定の回転は手動・固定角速度のため、動画は角速度差ではなく半径差の対比に留める。
...
- 内容（案）: 画面を左右 2 分割し、
  - 左: Shield が小さい半径で回転（Space 側）
  - 右: Shield が大きい半径で回転（Shift 側）
  - 中央下部に「Space → 半径縮小」「Shift → 半径拡大」のラベル
  - 両側とも同じ角速度（既定の手動回転）で回し、半径差だけを対比する
...
- `scripts/gen-how-to-play-video.mjs` を新設し、`pnpm run video:howtoplay:gen` から実行する。
  ...
  3. 先頭フレームを ffmpeg で `public/videos/how-to-play-poster.jpg` として書き出し（ffmpeg の利用は
     poster 抽出のみ）。
- **CI では実行しない**。ローカルで `pnpm run video:howtoplay:gen` を実行し、
...
- [x] `scripts/gen-how-to-play-video.mjs` と `pnpm run video:howtoplay:gen` が追加されている
